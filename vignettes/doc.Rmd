...
## Creation of a simulated classification dataset
- 50 samples of "A", and 50 samples of "B"
- 10 features of normal distribution noise

```{r create simu_data}
# Samples
df_simu_classif = data.frame(y=c(rep("A",50), rep("B",50)))
# Noise columns
set.seed(123)
noise_columns <- paste0("norm_noise", sprintf("%02d", 1:10))
df_simu_classif[noise_columns] <- lapply(noise_columns, function(column_name) {
    mean <- runif(1, min = -10, max = 10)
    sd <- runif(1, min = 1, max = 5)
    rnorm(nrow(df_simu_classif), mean = mean, sd = sd)
})

```

Now let's add 3 key features, the **t-test can detect the first 2, but not the last one**:
...
```{r}
rand_mean = runif(1, min = 2, max = 5)
rand_sd = runif(1, min = 1, max = 5)
mask_group = df_simu_classif[["y"]] == "A"
df_simu_classif[["diff_distrib01"]][mask_group] <- rnorm(sum(mask_group), mean = rand_mean, sd = rand_sd)
mask_group = df_simu_classif[["y"]] == "B"
df_simu_classif[["diff_distrib01"]][mask_group] <- rnorm(sum(mask_group), mean = -rand_mean, sd = rand_sd)

rand_mean = runif(1, min = 3, max = 5)
rand_sd = runif(1, min = 1, max = 5)
mask_group = df_simu_classif[["y"]] == "A"
df_simu_classif[["diff_distrib02"]][mask_group] <- rnorm(sum(mask_group), mean = rand_mean, sd = rand_sd)
rand_mean = runif(1, min = -5, max = -3)
rand_sd = runif(1, min = 1, max = 5)
mask_group = df_simu_classif[["y"]] == "B"
df_simu_classif[["diff_distrib02"]][mask_group] <- rnorm(sum(mask_group), mean = rand_mean, sd = rand_sd)
```
...
df_simu_classif$biDistrib = c(c_x, c_y)
```

We can see below how class A and class B are distributed accross the significant features: *diff_distrib01*, *diff_distrib02* and *biDistrib*.

```{r fig.width=10, fig.height=5}
p1 <- ggplot(df_simu_classif, aes(x=diff_distrib01, y=diff_distrib02, color=y)) +
    geom_point() + ggtitle("different normal distributions between A and B") + theme_bw()
p2 <- ggplot(df_simu_classif, aes(x=norm_noise01, y=biDistrib, color=y)) +
    geom_point() + ggtitle("normal bidistribution for A") + theme_bw()
...
```{r XAI.test 1 , echo=FALSE}
objXAI <- XAI.test(df_simu_classif, "y", simData = TRUE, simPvalTarget = 0.01)
```
...
First, we create a regression toy example of 100 samples, with 5 noise features, and a *y* feature computed from one of the noise feature. We will investigate if the feature used to compute *y* can be identified using correlation, linear modeling, or feature importance metrics.

## Creation of a regression simulated dataset
- 2 features of normal distribution noise
- 3 features of uniform distribution noise
- a *y* feature computed from one of the normal distribution noise

```{r}
set.seed(123)
df_simu_regr <- data.frame(norm_noise1=rnorm(100,1,1))
df_simu_regr$norm_noise1 <- rnorm(100,2,2)
df_simu_regr$norm_noise2 <- rnorm(100,-3,3)
df_simu_regr$unif_noise1 <- runif(100,0,1)
df_simu_regr$unif_noise2 <- runif(100,0,2)
df_simu_regr$unif_noise3 <- runif(100,-3,3)
```
...
The *y* feature is a parabole computed from the *norm_noise2* feature.
...
```{r}
regr_results <- XAI.test(df_simu_regr, "y", simData=T, simPvalTarget = 0.01)
...
```{r}
# featureImportanceXGBoost <- function(df, y="y", featImpAgr="mean"){
...
```{r}
set.seed(123)
results <- XAI.test(df_simu_classif,"y", simData = T,
